...
module mkXilinxIntMul32 (XilinxIntMul#(tagT, 32)) provisos (Bits#(tagT, tagSz));
  let m <- mkMul32;
  Fifo#(5, tagT) tags <- mkLatencyFifo(True, True);
  Fifo#(1, Bit#(64)) res <- mkBypassFifo(True, True);
  ...
  method Action req(Bit#(32) a, Bit#(32) b, XilinxIntMulSign sign, tagT tag);
    Tuple2#(Bool, Bool) x = case (sign)
      Signed: tuple2(True, True);
      Unsigned: tuple2(False, False);
      SignedUnsigned: tuple2(True, False);
    endcase;
    m.enq(tpl_1(x), a, tpl_2(x), b);
    tags.enq(tag);
  endmethod
  method Action deqResp;
    res.deq; tags.deq;
  endmethod
  method respValid = res.notEmpty && tags.notEmpty;
  method Bit#(64) product = res.first;
  method tagT respTag = tags.first;
endmodule
...
module mkXilinxIntDiv32 (XilinxIntDiv#(tagT, 32)) provisos (Bits#(tagT, tagSz));
  let d <- mkDiv32;
  Fifo#(TAdd#(DivStage, 2), tagT) tags <- mkLatencyFifo(True, True);
  Fifo#(1, Tuple2#(Bit#(32), Bit#(32))) res <- mkBypassFifo(True, True);
  ...
  method Action req(Bit#(32) dividend, Bit#(32) divisor, Bool signedDiv, tagT tag);
    d.enq(signedDiv, dividend, signedDiv, divisor);
    tags.enq(tag);
  endmethod
  method Action deqResp;
    res.deq; tags.deq;
  endmethod
  method Bool respValid = res.notEmpty && tags.notEmpty;
  method Bit#(32) quotient = tpl_1(res.first);
  method Bit#(32) remainder = tpl_2(res.first);
  method tagT respTag = tags.first;
endmodule

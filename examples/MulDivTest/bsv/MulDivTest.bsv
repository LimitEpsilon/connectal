...
(* synthesize *)
module mkMul(XilinxIntMul#(UserTag, 32));
   let m <- mkXilinxIntMul32;
   return m;
endmodule

(* synthesize *)
module mkDiv(XilinxIntDiv#(UserTag, 32));
   let m <- mkXilinxIntDiv32;
   return m;
endmodule
...
module mkMulDivTest(MulDivTest);
   ...
   // mul/div units
   XilinxIntMul#(UserTag, 32) mulUnit <- mkMul;
   XilinxIntDiv#(UserTag, 32) divUnit <- mkDiv;
